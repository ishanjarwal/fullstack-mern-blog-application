...
const ArticleListHome = () => {
    ...
    async function getPosts() {
        await axios.get('http://localhost:8080/api/posts')
            .then(val => {
                const limitedPosts = val.data.filter(el > indexOf(el) < 9)
    ...
    return (
        <section className='flex items-center justify-center w-full py-8'>
            <div className='max-w-6xl w-full h-full grid md:grid-cols-3 xs:grid-cols-2 grid-cols-1 md:gap-8 gap-4 px-4 xl:px-0 justify-items-center'>
                {posts.length > 0 && posts.map((el, index) => {
                    return (
                        <Card key={index} info={el} />
                    )
                })}
            </div>
        </section>
